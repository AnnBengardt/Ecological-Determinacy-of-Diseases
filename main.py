...
def ml_model():
    ...
    st.header('Самая результативная модель по итогам обучения для определения предрасположенности человека к социально значимым заболеваниям на основе района проживания в Москве')
    st.subheader("Так как в рамках проведения социологического опроса были получены данные не по всем социально значимым заболеваниям, то модель оценивает предрасположенность только для нескольких из них")
    st.subheader("Дисклеймер: просьба не относится к получаемым результатам слишком серьёзно и не использовать их для самодиагностики! Высокий процент предрасположенности не означает, что Вы больны или обязательно заболеете. " +
...
def main():
    ...
    option = st.sidebar.radio(['Результаты корреляционно-регрессионного анализа', 'Предрасположенность к социально значимым заболеваниям на основе района проживания', 'Экорейтинг районов Москвы'])
